    ...
    <div className="form-section animate-fade-in" style={{ animationDelay: '0.2s' }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div className="md:col-span-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-1.5 sm:gap-2 mt-2">
          ...
          <div className="flex flex-col sm:flex-row gap-2 mt-3">
            ...
            />
            <button type="button" onClick={addCustomResource} className="btn-secondary text-sm py-2">
              添加
            </button>
          </div>
          {data.resources.length > 0 && (
            <div className="flex flex-wrap gap-1.5 sm:gap-2 mt-3">
              {data.resources.map((resource, index) => (
                <span
                  key={index}
                  className="inline-flex items-center gap-1 px-2 sm:px-3 py-1 bg-accent text-primary rounded-full text-xs sm:text-sm"
                >
                  {resource}
                  <button
                    type="button"
      ...
      <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-gray-200">
        <h3 className="text-sm sm:text-lg font-semibold text-secondary mb-3 sm:mb-4">家庭诉求</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
          <div>
            <label className="form-label">升学目标 *</label>
            <textarea
              className="form-input min-h-[80px] sm:min-h-[100px]"
              placeholder="如：冲刺985/211"
